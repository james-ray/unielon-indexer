...
func (e *Explorer) swapDecode(tx *hcjson.TxRawResult, pushedData []byte, number int64) (*utils.SwapInfo, error) {
	...
	txRawResult0, err := e.node.GetRawTransactionVerbose(txhash0)
	if err != nil {
	...
	txRawResult1, err := e.node.GetRawTransactionVerbose(txhash1)
	if err != nil {
		return nil, chainNetworkErr
	}
...
func (e Explorer) swapCreateOrAdd(swap *utils.SwapInfo) error {
	...
	if info == nil {
		reservesAddress, _ := hcutil.NewAddressScriptHash([]byte(swap.Tick0+swap.Tick1), &chaincfg.MainNetParams)
